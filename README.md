...
Josa.tryParse('을/를');                    // Josa.eulReul (런타임 문자열 검증)
...
- 조사는 문자열 `'을/를'` 대신 enum `Josa.eulReul` — 잘못된 쌍(`'이/을'`)이 컴파일 타임에 차단된다. 런타임 문자열(사용자 입력 등)은 `Josa.tryParse`/`parse` 로 검증한다 (es-hangul 은 런타임 검증 없이 잘못된 쌍도 그대로 사용해 `'사과을'` 같은 결과가 나온다). 이름 규칙: 앞 형태 로마자 + 뒤 형태 로마자(첫 글자 대문자). `iGa` 이/가, `eulReul` 을/를, `eunNeun` 은/는, `euroRo` 으로/로, `waGwa` 와/과, `inaNa` 이나/나, `iranRan` 이란/란, `aYa` 아/야, `irangRang` 이랑/랑, `ieyoYeyo` 이에요/예요, `euroseoRoseo` 으로서/로서, `eurosseoRosseo` 으로써/로써, `eurobuteoRobuteo` 으로부터/로부터, `iraRa` 이라/라

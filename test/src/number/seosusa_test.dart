import 'package:dart_hangul/src/number/seosusa.dart';
...
void main() {
  group('seosusa', () {
    ...
    for (final c in validNumbers) {
      test('${c.num} - 순 우리말 서수사(${c.word})로 변환한다.', () {
        expect(seosusa(c.num), equals(c.word));
      });
    }
    ...
    for (final n in invalidNumbers) {
      test('$n - 유효하지 않은 숫자에 대해 오류를 발생시켜야 한다.', () {
        expect(
          () => seosusa(n),
          throwsA(isA<ArgumentError>().having((e) => e.message, 'message', '유효하지 않은 입력입니다. 1이상의 정수만 지원합니다.')),
        );
      });
    }
  });
